...
/**
	...
	*/
	...
	//My extension
    //Супер функция генерации массива содержащего айди столбцов excel
    function createColumnsArray($end_column, $first_letters = '',$startfrom = '')
    {
        $columns = array();
        $length = strlen($end_column);
        if(mb_strlen($startfrom) == 1 && mb_strlen($end_column) == 1){
            $letters = range($startfrom, 'Z');
        }else{
            $letters = range('A', 'Z');
        }
        ...
        if(!empty($startfrom)){
            foreach ($columns as $key=>$column){
                if($column == $startfrom){
                    $columns = array_slice($columns, $key, count($columns),true);
                    break;
                }
            }
        }
